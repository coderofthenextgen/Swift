    ...
    while (!done)
    {
        ...
        if (menu_open || menuAlpha > 0.0f)
        {
            ...
            ImGui::Begin("##GUI", NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground);
            {
                ...
                ImGui::PushFont(font);
                draw->AddText(ImVec2(p.x + 9.5, p.y + 7), ImColor(main_color), "Celex");
                draw->AddText(ImVec2(p.x + 9.5, p.y + 384), ImColor(255, 255, 255, 100), "Build:");
                draw->AddText(ImVec2(p.x + 41, p.y + 384), ImColor(main_color), ("Celex.gg | Beta"));
        ...
        if (IsGameOnTop("Roblox"))
        {
            ...
            std::string str = "Celex | " + std::to_string(static_cast<int>(io.Framerate)) + " FPS";
